...
import { Invoice, InvoiceResp } from '../../shared/types/consumables.types';
...
export class GenerateInvoicesComponent implements OnInit{
  ...
  protected registrationForm = new FormGroup({
    ...
    unitsConsumed: new FormControl(null, [Validators.required, Validators.min(0)]),
    customerId: new FormControl('', [Validators.required, Validators.pattern('^[a-zA-Z0-9]{5,}$')])
  });
  ...
  onSubmit() {
    if (this.registrationForm.valid) {
      const billingDetails: InvoiceResp = {
      ...
      if(billingDetails.periodEndDate < billingDetails.periodStartDate){
        ...
        this.registrationForm.get('periodEndDate')?.setErrors({ notGreater: true });
      ...
      this.invoiceGen.generateInvoice(billingDetails).subscribe({
        next: (response) => {
          ...
          const userName = this.invoice.customer.personalDetails.firstName + " " + this.invoice.customer.personalDetails.lastName;
          this.messageService.add({
            severity: 'success',
            summary: 'Registration Successful',
            detail: `Billing details submitted `+  ` to ` + userName + ` successfully.`,
          });
          setTimeout(() => {
          this.isloading = false
